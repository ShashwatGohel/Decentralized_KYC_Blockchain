...
const LoginPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!currentAccount) {
                 setError('Identity Wallet Required. Connecting to MetaMask...');
                 try {
                     await connectWallet();
            ...
            <div className="glass" style={{ 
                ...
                {error && (
                    <div className="glass" style={{ 
                        ...
                        <div style={{ fontWeight: 700, marginBottom: '0.3rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                            {error.includes('Wallet mismatch') ? <><Lock size={16}/> SECURITY CHECK</> : (error.includes('successful') ? <><CheckCircle size={16} /> SUCCESS</> : <><AlertTriangle size={16} /> ACTION REQUIRED</>)}
                        </div>
                        {error}
